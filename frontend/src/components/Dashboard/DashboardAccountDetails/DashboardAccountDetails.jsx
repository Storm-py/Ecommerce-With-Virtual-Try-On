...
const DashboardAccountDetails = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:4000/api/v1/users/update-profile-image", {
        method: "PATCH",
        credentials: "include",
        body: fd,
      })
  ...
  const handleDashboardAccountDetails = async (e) => {
    e.preventDefault()
    try {
      const response = await fetch(`http://localhost:4000/api/v1/users/update-account-details`, {
        method: "PATCH",
        headers: {
